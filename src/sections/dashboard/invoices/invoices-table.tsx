...
const statusMap: Record<InvoiceStatus, { color: string; label: string; }> = {
  draft: {
    color: 'warning.main',
    label: 'Draft'
  },
...
const getResourcesState = (params: { isLoading: boolean; error?: string; items: Invoice[] }): ResourcesState => {
  if (params.isLoading) {
    return 'loading';
  }
...
interface InvoicesTableProps {
  ...
  items?: Invoice[];
  onDeselectAll?: () => void;
  onDeselectOne?: (invoiceId: string) => void;
...
export const InvoicesTable: FC<InvoicesTableProps> = (props) => {
  const {
    count = 0,
    error,
    isLoading = false,
